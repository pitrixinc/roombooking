    ...
    useEffect(() => {
        const fetchUserData = async () => {
          if (id) {
           ...
           else if (!userData.isSuperAdmin  && userData.isTourist) {
                // User is not a creator or donor but is a mini admin, redirect to /dashboard
...
{/*
          <div class="space-y-2">
            ...
            <textarea id="af-submit-app-project-name" value={contactInfo} onChange={(e) => setContactInfo(e.target.value)} required type="text" class="py-2 px-3 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" rows="6" placeholder="Information on how backers can get in touch with the project team."></textarea>
          </div>
*/}
